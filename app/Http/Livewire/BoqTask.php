...
class BoqTask extends Component
{
    use AuthorizesRequests, NotificationManager;

    public $project, $boqs, $version = [], $show_version, $max_version, $check_approval = [], $task;
    public $adendum;
    ...
    public function render()
    {
        ...
        switch ($this->filter) {
            ...
            case 'waiting_for_approval':
                $boqList = $boqList->filter(function ($boq) {
                    if ($boq->rejected_by !== null) return false;
        ...
        return view('masterdata.boqs.task-index', [
    ...
    private function filterByPurchaseStatus($boqList, $purchased)
    {
        ...
        $filteredData = collect($dataArray)->filter(function ($item) use ($purchased) {
    ...
    public function hide_adendum()
    {
        ...
        return redirect()->route('boq.task.index', ['project' => $this->project->id, 'task' => $this->task->id]);
    }
    ...
    public function approve_all()
    {
        ...
        return redirect()->route('boq.task.index', ['project' => $this->project->id, 'task' => $this->task->id])->with('success', 'All Item has beed approved.');
    }
    ...
    public function reject_all()
    {
        ...
        return redirect()->route('boq.task.index', ['project' => $this->project->id, 'task' => $this->task->id])->with('danger', 'All Item has beed rejected.');
    }
    ...
    public function update(Request $request, $item, $project_id)
    {
        ...
        return redirect()->route('boq.task.index', ['project' => $this->project->id, 'task' => $this->task->id])->with('success', 'The Item has been updated successfully.');
    }
    ...
    public function destroy($item, $project_id)
    {
        ...
        return redirect()->route('boq.task.index', ['project' => $this->project->id, 'task' => $this->task->id])->with('success', 'The Item has beed deleted successfully.');
    }
