...
class ChooseItemGt extends Component
{
    use WithPagination;
    public $search;
    ...
    public function mount($id)
    {
        $this->prid = $id;
        // dd($id);
        $this->prequest = GudangTransfer::where("id", $id)->first();
        // $this->itemsarray1 = Item::all()->toArray();
        $prd = GudangTransferDetail::where("pr_id", $id)->get();
    ...
    public function render()
    {
        ...
        return view('livewire.choose-item-gt', compact(
            'items',
            'carts'
        ));
        ...
        foreach ($this->itemsarray as $key => $value) {
            if ($value["id"] == $id) {
                ...
                if ($value["count"]) {
                    ...
                    GudangTransferDetail::where("pr_id",$this->prid)->where("item_id",$value["id"])->update([
        ...
        foreach ($this->itemsarray as $key => $value) {
            if ($value["id"] == $id) {
                ...
                if ($value["count"] > 1) {
                    ...
                    unset($this->itemsarray[$key]);
                    GudangTransferDetail::where('pr_id',$this->prid)->where("item_id",$value["id"])->delete();
                }
            }
        }
    }
    ...
    {
        GudangTransferDetail::where('pr_id',$this->prid)->where('item_id',$this->itemsarray[$key]["id"])->delete();
        unset($this->itemsarray[$key]);
    }
    public function updateqty($key)
    {
        $item = $this->itemsarray[$key];
        GudangTransferDetail::where("pr_id",$this->prid)->where("item_id",$item["id"])->update([
            "qty" => $item["count"],
        ]);
    }
    ...
    // public function render()
    // {
    //     return view('livewire.choose-item-gt');
    // }
}
