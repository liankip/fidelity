...
use App\Models\InventoryUsageDetail;

use Livewire\WithPagination;
use Illuminate\Support\Carbon;
use Livewire\Component;

class ChooseItemInventoryUsage extends Component
{
    use WithPagination;
    public $search;
    ...
    public function mount($id)
    {
        $this->prid = $id;
        // dd($id);
        $this->prequest = InventoryUsages::where("id", $id)->first();
        // $this->itemsarray1 = Item::all()->toArray();
        $prd = InventoryUsageDetail::where("pr_id", $id)->get();
    ...
    public function render()
    {
        ...
        return view('livewire.choose-item-inventory-usage', compact(
            'items',
            'carts'
        ));
        ...
        foreach ($this->itemsarray as $key => $value) {
            if ($value["id"] == $id) {
                ...
                if ($value["count"]) {
                    ...
                    InventoryUsageDetail::where("pr_id",$this->prid)->where("item_id",$value["id"])->update([
        ...
        foreach ($this->itemsarray as $key => $value) {
            if ($value["id"] == $id) {
                ...
                if ($value["count"] > 1) {
                    ...
                    unset($this->itemsarray[$key]);
                    InventoryUsageDetail::where('pr_id',$this->prid)->where("item_id",$value["id"])->delete();
                }
            }
        }
    }
    ...
    {
        InventoryUsageDetail::where('pr_id',$this->prid)->where('item_id',$this->itemsarray[$key]["id"])->delete();
        unset($this->itemsarray[$key]);
    }
    public function updateqty($key)
    {
        $item = $this->itemsarray[$key];
        InventoryUsageDetail::where("pr_id",$this->prid)->where("item_id",$item["id"])->update([
            "qty" => $item["count"],
        ]);
    }
    ...
    // public function render()
    // {
    //     return view('livewire.choose-item-inventory-usage');
    // }
}
