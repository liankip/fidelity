    ...
    <div class="row">
        <div class="col-lg-12 margin-tb">
            <div class="pull-left d-lg-flex justify-content-between">
                <div class=" ">
                    <h3 class="text-black">Daftar Spreadsheet Pengajuan BOQ Retail - {{ $project->name }}</h3>
                </div>
            </div>
            <hr>
    ...
    <div class="card">
        <div class="card-body">
            ...
            <ul class="nav nav-tabs">
                ...
                <li class="nav-item">
                    <button class="nav-link {{ $status == 1 ? 'active' : '' }}"
                        wire:click="updateStatus(1)">Approved</button>
                </li>
            </ul>

            <div class="d-flex justify-content-end mb-4">
                <a href="{{ route('boq.create-boq', $project->id) }}" class="btn btn-primary">Buat BOQ</a>
            </div>
            <table class="table">
                                ...
                                @hasanyrole('manager|top-manager|it')
                                    @if ($submitted->status == 'Submitted')
                                        <button class="btn btn-sm btn-outline-danger"
                                            wire:click="delete({{ $submitted->id }})">Delete</button>
                                    @endif
                                @endhasanyrole
