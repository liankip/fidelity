        ...
        <div class="col-lg-12 margin-tb">
            <div class="pull-left">
                <div class="d-flex">
                    ...
                    <h2 class="my-auto">IBPR - {{ $ibpr->name }}</h2>
                </div>
        ...
        <div class="card mt-2">
            <div class="card-body">
                <div class="d-flex justify-content-end">
                    <a href="{{ route('k3.ibpr.createList', $ibpr->id) }}">
                ...
                <div class="overflow-x-max">
                    <table class="table table-bordered fs-6 mt-4">
                        ...
                        <tbody>
                            @forelse ($ibprLists as $key => $ibprList)
                                @php
                                    $data = json_decode($ibprList->data);
                                @endphp
                                <tr wire:key="ibpr-{{ $ibprList->id }}"
                                    style="font-size: 13px; background-color: white">
                                    <td>{{ $key + 1 }}</td>
                                    <td>{{ $ibprList->activity }}</td>
                                    <td>
                                        <a href="{{route('k3.ibpr.editList', [$ibpr->id, $ibprList->id])}}" class="btn btn-sm btn-success">Edit</a>
                                        <button type="button" wire:click='deleteModal({{ $ibprList->id }})'
                                            class="btn btn-sm mt-1 btn-danger">Hapus</button>
                                    </td>
                                </tr>
                                @if ($selectDelete === $ibprList->id)
                                    <div
                                                ...
                                                <div class="modal-content">
                                                    ...
                                                    <div class="modal-footer">
                                                        ...
                                                        <button type="button" class="btn btn-primary"
                                                            wire:click="destroy({{ $ibprList->id }})">Submit</button>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
